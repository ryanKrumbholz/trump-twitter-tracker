...
def count_tweets():
    tweets = api.user_timeline('@realDonaldTrump', include_rts=False) #tweets from Donald Trump's twitter
    ...
    today = time.gmtime()[2] #today's day of the month (numerically)

    def tweet_oncommand():
        current_time = str(time.gmtime()[3]) + 'hrs ' + str(time.gmtime()[4]) + 'mins ' + str(time.gmtime()[5]) + 'secs'
        for mention in reversed(mentions): #iterating through all unseen mentions
            ...
            if '#count' in mention.text:
                ...
                api.update_status("@" + user + " Trump has tweeted " + str(count) + " times so far today as of " + current_time + "!")
    ...
    def tweet_count_midnight():
        ...
        if midnight:
            ...
            api.update_status("Yesterday (" + yesterday.strftime('%m/%d/%Y') + ") Trump tweeted " + str(count) + " times!")
